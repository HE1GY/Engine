  ...
  public:
    ExampleLayer() : Layer("ExampleLayer"), m_camera(-1.6, 1.6, -0.9, 0.9), m_square_pos(1.0f)
    {
        ...
        m_texture_2 = Engine::Texture2D::Create(
            std::filesystem::current_path().parent_path().concat("/../Sandbox/assets/textures/ILTQq.png"));
        m_texture = Engine::Texture2D::Create(std::filesystem::current_path().parent_path().concat(
            "/../Sandbox/assets/textures/wallpaperflare.com_wallpaper.png"));
    }
